 ...
 */
for _ in 0..<10 {
    let foo = 55 // diff scope only works in the for loop
...
var x = 10                    // x is defined here — global to the loop
for _ in 0..<10 {
    x += 1                   // x is modified inside the loop
    print("The value of x is \(x)") // x is used inside the loop
}
print("The final value of x is \(x)") // x is used outside the loop
...
func greeting(greeting: String?, name: String) {
    if let greeting = greeting { // variable shadowing here
        print("\(greeting), \(name).")
    } else {
        print("Hello, \(name).")
    }
...
class Car {
    ...
    init(make: String, model: String, year: Int) {
        self.make = make     // parameter shadowing: 'make' parameter shadows 'make' property
        self.model = model   // same here
        self.year = year     // and here
    }
}
