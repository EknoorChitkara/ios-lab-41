...
func checkAge(from input: String) -> Int? {
...
func costOf(item name: String) -> Double? {
    if let itemStock = stock[name], itemStock > 0 {
        return prices[name]
    } else {
        return nil
    }
}
let costOfBanana: Double? = costOf(item: "Banana")
print(costOfBanana ?? "Item is out of stock")
